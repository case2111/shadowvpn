  ...
  while (ctx->running) {
...
#endif
	// read tun msg, down stream nat, encrypt msg, send to another side, me add
    if (FD_ISSET(ctx->tun, &readset)) 
    {
      ...
      if (usertoken_len) 
      {
        ...
        {
          // do NAT for downstream, is server mode
      ...
      if (ctx->remote_addrlen) 
      {
		...
		//	send msg to another side(client/server), is tunnel msg.me add
    ...
    // receive msg from local udp socet, decrypt msg, up stream nat, write to tun, me add
    for (i = 0; i < ctx->nsock; i++) 
    {
      int sock = ctx->socks[i];
      if (FD_ISSET(sock, &readset)) 
      {
        ...
        // receive msg from local udp socket, me add
        ...
        else 
        {
          ...
          logf("need write %ld byte into tun", r - SHADOWVPN_OVERHEAD_LEN - usertoken_len);
          if (-1 == write(ctx->tun,
                              ctx->tun_buf + SHADOWVPN_ZERO_BYTES + usertoken_len,
                              r - SHADOWVPN_OVERHEAD_LEN - usertoken_len))
          {
          	logf("write %ld byte into tun failed", r - SHADOWVPN_OVERHEAD_LEN - usertoken_len);
            if (errno == EAGAIN || errno == EWOULDBLOCK) 
            {
              // do nothing
            } 
            ...
            {
              // just log, do nothing
              err("write to tun EPERM");
            }
            else if (errno == EINTR)
            {
            	err("write to tun EINTR");
            }
            else if (errno == EINVAL)
            {
            	err("write to tun EINVAL");
            }
            else 
            {
              err("write to tun");
          ...
          {
          	logf("write %ld byte into tun success", r - SHADOWVPN_OVERHEAD_LEN - usertoken_len);
          }
        }
      }
    }
